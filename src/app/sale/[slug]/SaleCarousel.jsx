...
function getBadge(sale) {
  ...
  if (start && now < start)
    return { text: `Starts ${start.toLocaleDateString("en-IN", { day: "numeric", month: "short" })}`, tone: "upcoming" };
        ...
        <div className="flex -ml-6">
          {sales.map((sale) => {
            ...
            return (
              ...
              <div key={sale.id} className="min-w-0 flex-[0_0_100%] md:flex-[0_0_50%] pl-6">
                <Link
                  href={`/sale/${sale.slug}`}
                ...
                >
                  ...
                  <span className={`self-start rounded-full px-3 py-1 text-[12px] font-extrabold uppercase tracking-tight border ${
                    badge.tone === "live" ? "bg-green-500/90 border-green-300 text-white" :
                  ...
                  <div className="flex items-end justify-between w-full">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <h3 className="text-white tracking-[-0.68px]" style={{ fontFamily: "var(--font-display)" }}>
                        {sale.saleHeading || sale.title}
                      </h3>
                      {sale.saleSubtitle && (
                        <p className="text-[15px] font-semibold leading-6 max-w-[350px]" style={{ color: "#eeebfd" }}>
                          {sale.saleSubtitle}
                        </p>
                      )}
                    </div>
